...
# logger = log.getLogger("MER_ZoobotMorphoUtils")
...
# def apply_zoobot_selection_cuts(df: pd.DataFrame):
#     df = add_zoobot_selection_flags(df)
#     df['DETAILED_MORPHOLOGY_READY'] = passes_zoobot_selection_cuts(df)
#     return df[df['DETAILED_MORPHOLOGY_READY']]


def passes_final_zoobot_selection_cuts(dictlike: dict):
    if 'LOW_SEGMENTATION_AREA_VIS_FLAG' not in dictlike.keys():
        dictlike = add_zoobot_selection_flags_from_final_catalog(dictlike)  # adds all selection cut columns I need
...
def passes_module_zoobot_selection_cuts(dictlike: dict):
    if 'LOW_SEGMENTATION_AREA' not in dictlike.keys():
        dictlike = add_zoobot_selection_flags_from_module_catalog(dictlike)  # adds all selection cut columns I need
        # print(dictlike)
    # print(~(dictlike['LOW_SEGMENTATION_AREA'] | dictlike['LOW_SEGMENTATION_FLUX']))
    # print(~np.logical_or(dictlike['LOW_SEGMENTATION_AREA'], dictlike['LOW_SEGMENTATION_FLUX']))
    return ~np.logical_or(dictlike['LOW_SEGMENTATION_AREA'], dictlike['LOW_SEGMENTATION_FLUX'])


def add_zoobot_selection_flags_from_module_catalog(df: pd.DataFrame):
    """
    Within the pipeline, Zoobot is run *before* we calculate all the useful filter columns (e.g. GAIA_ID).
    So we need to do an early rough filter to select relevant sources, run Zoobot, and then do a full filter in a later stage.
    The early rough filter uses only two columns:
    - SEGMENTATION_AREA -> ISOAREA
    - FLUX_VIS_APER -> FLUX_TOT (very rough! It's basically the sum of all pixels assigned to the given source)
 
    Args:
        df (pd.DataFrame): sources
    """
    df['LOW_SEGMENTATION_AREA'] = df['ISOAREA'] < 1200   # TODO check ISOAREA is the same as SEGMENTATION_AREA

    # hmm, this seems to have totally different units to FLUX_VIS_APER. Temporarily removed TODO
    df['LOW_SEGMENTATION_FLUX'] = False
    # df['LOW_SEGMENTATION_FLUX'] = df['FLUX_TOT'] < 22.90867652  # TODO check this value is the same or larger than FLUX_VIS_APER!
    return df

def add_zoobot_selection_flags_from_final_catalog(df: pd.DataFrame):  # works on either dataframe or dict
    """
    Ultimately we want to apply these cuts, using the final catalog columns:
        GAIA_ID, needed for selecting which galaxies to run Zoobot
        SEGMENTATION_AREA, needed for selecting which galaxies to run Zoobot and potentially for estimating R_MAX
        FLUX_VIS_APER, for identifying galaxies with bad flux measurements to skip
        SPURIOUS_PROB, for identifying galaxies with high spurious probability to skip
        FLUX_SEGMENTATION, for identifying galaxies with low flux to skip
        KRON_RADIUS, useful for estimating R_MAX (R_MAX estimate only needed outside the MER pipeline, post-hoc)
    """
    # with dataframe, useful to have these broken out as columns for counting how many sources we lose with each cut

    # no stars
    # this column only exists in final catalog, and so this filter must be applied post-hoc in validation step
    ...
    df['LOW_SEGMENTATION_AREA'] = df['SEGMENTATION_AREA'] < 0.1  # TODO I might have messed this up, should be 1200?!
...
def estimate_source_r_max(source):
    """
    Estimate the maximum radius of a source based on its properties using a linear regression model.
    Parameters:
    - source: astropy Table row, the source for which to estimate the maximum radius,
    Returns:
    - r_max_estimate: float, the estimated maximum radius of the source in pixels
    """
    intercept = -0.35048866
    ...
    r_max_estimate_pixels = 10 ** log_r_max_estimate + 2 # add 2 pixels to ensure the cutout is large enough to contain the source

    if r_max_estimate_pixels < 10:
        r_max_estimate_pixels = 10 #minimum cutout size for faint sources

    return r_max_estimate_pixels
...
def convert_final_cat_to_within_pipeline_cat(df, vis_tile_loc):
    ...
    required_columns = ['SEGMENTATION_AREA', 'KRON_RADIUS', 'ELLIPTICITY', 'RIGHT_ASCENSION', 'DECLINATION']
    valid_rows = df[required_columns].notna().all(axis=1)
    df = df.loc[valid_rows].copy()  
    N_dropped = len(df) - len(df.loc[valid_rows])
    print(f"Dropped {N_dropped} sources with missing required columns for R_MAX estimation and pixel center calculation")
