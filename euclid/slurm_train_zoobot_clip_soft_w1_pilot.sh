#!/bin/bash
#SBATCH --job-name=euclid_softw1_pilot
#SBATCH --output=/n03data/huertas/euclid/sfh_clip/edfn_100k/train_soft_w1_pilot_%j.out
#SBATCH --error=/n03data/huertas/euclid/sfh_clip/edfn_100k/train_soft_w1_pilot_%j.err
#SBATCH --nodes=1
...
# A controlled 10k-pair comparison with the exact-pair pilot. The model and
# optimizer settings stay fixed; only 25% of each target is distributed among
# the eight closest within-batch SFHs under Wasserstein-1 distance.
#
# Usage:
...
OUTPUT_DIR=${4:-${BASE_DIR}/training_pilot_transformer_median_soft_w1}
...
python -u -m euclid.train_zoobot_clip \
    ...
    --run-name euclid_vis_sfh_transformer_median_soft_w1_pilot \
    --no-sample-posterior \
    --max-pairs 10000 \
    --batch-size 128 \
    --queue-size 0 \
