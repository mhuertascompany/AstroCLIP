#!/bin/bash
#SBATCH --job-name=euclid_bright_test
#SBATCH --output=/n03data/huertas/euclid/sfh_clip/edfn_100k/train_bright_test_%j.out
#SBATCH --error=/n03data/huertas/euclid/sfh_clip/edfn_100k/train_bright_test_%j.err
...
if [[ $# -lt 1 ]]; then
    echo "Usage: sbatch $0 MAX_VIS_MAG" >&2
    exit 2
fi
...
MAX_VIS_MAG=$1
TAG=${MAX_VIS_MAG/./p}
OUTPUT=${BASE}/training_test_transformer_median_vislt${TAG}
...
python -u -m euclid.train_zoobot_clip \
    ...
    --output-dir "${OUTPUT}" \
    --run-name "euclid_vis_sfh_transformer_median_vislt${TAG}_test" \
    --no-sample-posterior \
    --max-pairs 1024 \
    --batch-size 128 \
    --queue-size 0 \
