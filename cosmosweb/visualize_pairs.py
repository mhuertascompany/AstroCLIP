...
def render_sfh(ax, sfh_log: np.ndarray, time_grid: np.ndarray):
    """
    Plot SFH as a step function: SFR (M_sun/yr) vs lookback time (Gyr).
    sfh_log is in log10(SFR + eps) space.
    """
    sfr = 10.0 ** sfh_log - SFH_EPS          # back to physical SFR
    sfr = np.maximum(sfr, SFH_EPS)            # guard against negatives from float precision
    time_gyr = time_grid / 1e3                # Myr → Gyr

    ax.step(time_gyr, sfr, where='post', color='steelblue', lw=1.2)
    ax.fill_between(time_gyr, sfr, step='post', alpha=0.25, color='steelblue')

    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlim(time_gyr[0] * 0.9, time_gyr[-1] * 1.1)
...
def make_page(fig, pairs: list, time_grid: np.ndarray,
              img_mean: float, img_std: float):
    ...
    for i, (img, sfh, z, gid) in enumerate(pairs):
        ...
        render_sfh(ax_sfh, sfh, time_grid)

        ax_img.set_title(f'id={gid}\nz={z:.2f}', fontsize=6, pad=2)
...
def main():
    ...
    images, sfhs, time_grid, redshifts, gal_ids, img_mean, img_std = \
    ...
    with PdfPages(args.output) as pdf:
        for page in range(args.n_pages):
            ...
            make_page(fig, pairs, time_grid, img_mean[0], img_std[0])
            fig.suptitle(
