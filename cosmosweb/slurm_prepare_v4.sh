#!/bin/bash
#SBATCH --job-name=cweb_prepare_v4
#SBATCH --output=/n03data/huertas/COSMOS-Web/cosmosweb_clip/prepare_v4.out
#SBATCH --error=/n03data/huertas/COSMOS-Web/cosmosweb_clip/prepare_v4.err
#SBATCH --nodes=1
...
# Changes vs v2:
#   - Normalise the 9 CIGALE bins BEFORE interpolation (grid-independent).
#     v2 normalised after interpolation, distorting values with grid density.
#   - fill_value: (sfr_frac[0], 0) instead of 0 — extends most-recent CIGALE
#     bin to t=0 rather than zeroing it.
#   - SFH_T_FRAC: linspace(0, 1, 50) retained (same as v2).
# Output is cosmosweb_dataset_v4.h5; previous versions left untouched.
...
python cosmosweb/prepare_dataset.py \
    ...
    --output      ${OUTPUT_DIR}/cosmosweb_dataset_v4.h5 \
    --stamp_size  64
